...
// AuthServiceImpl реализация
type AuthServiceImpl struct {
	repo      repository.UserRepository
	jwtSecret string
}

// NewAuthService создаёт сервис с внутренним секретом JWT
...
// Генерация случайного секретного ключа для JWT
func generateRandomSecret() string {
	b := make([]byte, 32)
	_, err := rand.Read(b)
	if err != nil {
		panic("failed to generate JWT secret") // можно обработать иначе
	}
	return base64.StdEncoding.EncodeToString(b)
}

// Получить секрет JWT (для мидлвары)
...
// Проверка уникальности ошибки PostgreSQL
